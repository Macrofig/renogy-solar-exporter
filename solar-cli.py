...
# TODO: use this to read output when connection fails
def parse_discovery_config(output):
...
# Updates the config, useful when auto detecting mac address
...
def attempt_connection():
    ...
    try:
        ...
        output = "TODO, put the output of the command here!"
        # incremental backoff on future attempts
        return auto_discover(output)


def auto_discover(output):
    addresses = parse_discovery_config(output)
    if len(addresses) > 0:
        config = {"mac_addr": addresses[0]}
...
@app.route("/health")
def health():
    # TODO If mac address was not passed, use first returned from discovery
    if not config.mac_addr:
        found = attempt_connection()
